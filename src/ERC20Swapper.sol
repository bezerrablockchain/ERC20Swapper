...
contract ERC20Swapper is IERC20Swapper {
    ...
    function swapEtherToToken(address token, uint minAmount) external payable returns (uint amountOut) {
        ...
        bytes memory message = abi.encodeWithSignature("exactInputSingle((address,address,uint24,address,uint256,uint256,uint160))", params);
        (bool sent, bytes memory data) = address(swapRouter).call{value: msg.value}(message);
